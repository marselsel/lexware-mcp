...
Built with the [Skybridge](https://docs.skybridge.tech) framework as an **MCP App**, so UI
views can be added later without re-architecting. **Bring your own Lexware API key** — the
server is single-tenant per deployment and never stores anyone else's credentials.

**What makes this one different:** it's designed to run **remotely/hosted** (e.g. Google Cloud
Run) and connect as an **OAuth-protected custom connector** in the Claude app, **claude.ai
web**, and **ChatGPT** — nothing to install locally. If you'd rather run a local stdio server,
the excellent [lazyants/lexware-mcp-server](https://github.com/lazyants/lexware-mcp-server) and
[JannikWempe/mcp-lexware-office](https://github.com/JannikWempe/mcp-lexware-office) are great
alternatives.
